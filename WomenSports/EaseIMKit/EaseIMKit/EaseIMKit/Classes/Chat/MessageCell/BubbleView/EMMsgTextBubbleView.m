...
//    self.textLabel.font = [UIFont systemFontOfSize:_viewModel.contentFontSize];
    self.textLabel.font = [UIFont fontWithName:@"SFProRounded-Regular" size:_viewModel.contentFontSize];
    ...
    if (self.direction == EMMessageDirectionSend) {
        self.textLabel.textColor = _viewModel.contentSendFontColor;
        [self.textLabel Ease_makeConstraints:^(EaseConstraintMaker *make) {
            make.left.equalTo(self.ease_left).offset(20);
            make.right.equalTo(self.ease_right).offset(-20);
        }];
    } else {
