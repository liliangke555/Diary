...
-(void)setup{
    ...
    [self.calendarWeekView mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [self.collectionView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.calendarWeekView.mas_bottom);
...
//    self.calendarWeekView.frame = CGRectMake(0, 0, CGRectGetWidth(self.frame), 50);
}
#pragma mark --左滑手势--
...
//    self.calendarHeader.dateStr = [NSString stringWithFormat:@"%ld年%ld月",monthModel.year,monthModel.month];
    ...
    for (int i = 0; i <42; i++) {
        ...
        if (i >= firstWeekday && i < (firstWeekday + totalDays)) {
            ...
            //标识是未来
            if ((monthModel.year > [[NSDate date] dateYear])) {
                model.isFuture = YES;
            }
        }
         //下月的日期
        ...
        if ((monthModel.month == self.month) && (monthModel.year == self.year)) {
            if (i == self.day + firstWeekday - 1) {
...
-(LXCalendarWeekView *)calendarWeekView{
    if (!_calendarWeekView) {
        _calendarWeekView =[[LXCalendarWeekView alloc] init];
        _calendarWeekView.weekTitles = @[@"Sun",@"Mon",@"Tues",@"Wed",@"Thurs",@"Fri",@"Sat"];
    }
...
-(UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        flow.itemSize = CGSizeMake(CK_WIDTH/7.0, 50);
