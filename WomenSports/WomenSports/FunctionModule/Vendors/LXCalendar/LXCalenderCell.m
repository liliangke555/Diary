...
- (void)awakeFromNib {
    [super awakeFromNib];
    // Initialization code
    [self.label setFont:KSFProRoundedMediumFont(14)];
...
-(void)setModel:(LXCalendarDayModel *)model{
    ...
    if (model.isNextMonth || model.isLastMonth) {
        ...
        if (model.isFuture) {
            self.userInteractionEnabled = NO;
            [self.label setTextColor:K_TextGrayColor];
        } else {
            self.userInteractionEnabled = YES;
            if (model.isSelected) {
                [self.label.layer setCornerRadius:CGRectGetWidth(self.label.bounds) / 2.0f];
                self.label.layer.masksToBounds = YES;
                self.label.backgroundColor = model.selectBackColor?model.selectBackColor:[UIColor greenColor];
                [self.label setTextColor:K_WhiteColor];
                if (model.isHaveAnimation) {
                     [self addAnimaiton];
                }
               
            } else {
                self.label.textColor = model.currentMonthTitleColor?model.currentMonthTitleColor:[UIColor k_colorWithHex:0x000000FF];
                if (model.isToday) {
