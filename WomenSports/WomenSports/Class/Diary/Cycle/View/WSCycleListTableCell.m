...
@interface WSCycleListTableCell ()
...
@implementation WSCycleListTableCell
...
- (void)setModel:(WSCycleDetailModel *)model {
    _model = model;
    if (model) {
        [self.pImageView sd_setImageWithURL:[NSURL URLWithString:model.data9] placeholderImage:[UIImage imageNamed:@"set_header_icon"]];
        ...
        if (model.data6.length > 0) {
            ...
            [imageView mas_makeConstraints:^(MASConstraintMaker *make) {
                make.top.left.equalTo(self.imageBackView).insets(UIEdgeInsetsZero);
                make.width.height.mas_equalTo(219*CK_HEIGHT_Sales);
                make.bottom.equalTo(self.imageBackView.mas_bottom).mas_offset(-20);
            }];
        ...
        if ([model.userId isEqualToString:kUser.uid]) {
            [self.moreButton setHidden:YES];
        } else {
            if ([model.data8 integerValue] == 1) {
                [self.moreButton setHidden:YES];
            } else {
