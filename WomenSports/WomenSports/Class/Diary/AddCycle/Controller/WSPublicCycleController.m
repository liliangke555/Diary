...
@property (nonatomic, strong) WSCycleAddOrEditRequest *saveRequest;
@end

@implementation WSPublicCycleController
...
- (void)uploadImage {
    ...
    CKWeakify(self);
    [MBProgressHUD showLoadingWithMessage:@"Picture uploading..."];
...
- (void)saveCycleRequest {
    ...
    [self.saveRequest asyncRequestWithsuccessHandler:^(BaseResponse * _Nonnull response) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"WSPublicCycle" object:nil];
...
- (void)setupView {
    ...
    [textView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(lineView.mas_bottom).mas_offset(20*CK_HEIGHT_Sales);
        make.left.right.equalTo(self.view).insets(UIEdgeInsetsMake(0, 15, 0, 15));
        make.height.mas_equalTo(178*CK_HEIGHT_Sales);
    }];
    ...
    }
    {
        ...
        [synLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(imageButton.mas_bottom).mas_offset(30*CK_HEIGHT_Sales);
            make.left.equalTo(self.view.mas_left).mas_offset(15);
        }];
...
- (void)turnButtonAction:(UIButton *)sender {
    ...
    if (sender.isSelected) {
        self.saveRequest.data8 = @"1";
        self.saveRequest.data9 = kUser.anonymousHeaderUrl;
        self.saveRequest.data10 = kUser.anonymousName;
    } else {
        self.saveRequest.data8 = @"0";
        self.saveRequest.data9 = kUser.headerUrl;
        self.saveRequest.data10 = kUser.name;
    }
}
...
- (WSCycleAddOrEditRequest *)saveRequest {
    if (!_saveRequest) {
        _saveRequest = [WSCycleAddOrEditRequest.alloc init];
        _saveRequest.data8 = @"0";
