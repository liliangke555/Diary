...
- (void)clickPublish {
    ...
    WSPublicCycleController *v = [[WSPublicCycleController alloc] init];
    [[UIViewController currentNavigatonController] pushViewController:v animated:YES];
}

#pragma mark - CYLPlusButtonSubclassing

+ (id)plusButton {
    WSPushButton *button = [WSPushButton buttonWithType:UIButtonTypeCustom];
    // 图片尺寸：56*56、67*66、49*48（凸出 15）
    UIImage *normalButtonImage = [UIImage imageNamed:@"tabbar_mine_icon"];
    UIImage *hlightButtonImage = [UIImage imageNamed:@"tabbar_mine_selected"];
...
//    button.titleLabel.font = [UIFont systemFontOfSize:10 weight:UIFontWeightBold];
    ...
    //自定义宽度
    button.frame = CGRectMake(0.0, 0.0, 52, 52);
...
-(void)plusChildViewControllerButtonClicked:(UIButton<CYLPlusButtonSubclassing> *)sender{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"tabbarPlustBtnNotifi" object:nil userInfo:nil];
}
...
//
//    WSPublicCycleController *v = [[WSPublicCycleController alloc] init];
//    WSNavigationController *nav = [WSNavigationController.alloc initWithRootViewController:v];
//    return nav;
//}
